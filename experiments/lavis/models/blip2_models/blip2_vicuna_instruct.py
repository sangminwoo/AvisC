...
class Blip2VicunaInstruct(Blip2Base):
    ...
    def forward(self, samples):
        ...
        ###
        # if masking strategy? change here (maybe?)
        ###
    ...
    def generate(
        ...
        with self.maybe_autocast():
            ...
            outputs = self.llm_model.generate(
                ...
                use_avisc=use_avisc if use_avisc is not None else None,
                layer_gamma=layer_gamma if layer_gamma is not None else None,
                masking_scheme=masking_scheme if masking_scheme is not None else None,
                lamb=lamb if lamb is not None else None,
                model_name="BLIP",
                use_m3id=use_m3id if use_m3id is not None else None,
            )
